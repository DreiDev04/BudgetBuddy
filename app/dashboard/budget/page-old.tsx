...
const Page = () => {
  ...
  const accountData =
    selectedBudget?.map((budget) => ({
      ...
      expenses: budget.expenses?.reduce((sum, exp) => sum + exp.amount, 0) ?? 0,
    })) ?? [];

  const spendingData =
    selectedBudget?.flatMap(
      (budget) =>
      ...
      <Card className="flex items-center gap-4 border-b p-5 justify-center lg:justify-end sm:flex-row">
        <div className="flex items-center gap-2">
          ...
          <Input
            ...
            placeholder={`$${(selectedBudgetId === "all"
              ? userBudget.reduce((sum, b) => sum + b.budget, 0) // Sum all budgets
              : userBudget.find((b) => b._id === selectedBudgetId)?.budget || 0
            ) // Find selected budget
              .toLocaleString()}`}
